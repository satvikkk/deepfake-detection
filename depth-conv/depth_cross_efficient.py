...
class ImageEmbedder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Load EfficientNet pretrained model and remove specified blocks
        self.efficient_net = EfficientNet.from_pretrained('efficientnet-b0')
        self.efficient_net.delete_blocks(efficient_block)
        self.efficient_block = efficient_block
        
        # Allow all parameters in EfficientNet to be trainable
        ...
        # Define depth-wise separable convolution for patch embedding
        self.to_patch_embedding = nn.Sequential(
            nn.Conv2d(channels, channels, kernel_size=patch_size, stride=patch_size, groups=channels),
            nn.Conv2d(channels, dim, kernel_size=1),  # Point-wise convolution
    ...
    def forward(self, img):
        # Extract features from EfficientNet up to specified block
        x = self.efficient_net.extract_features_at_block(img, self.efficient_block)
        
        # Convert features to patch embeddings
...
class CrossEfficientViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Define S-branch and L-branch image embedders
        ...
        # Define Multi-Scale Encoder module
        ...
        # Define MLP heads for S-branch and L-branch
        self.sm_mlp_head = nn.Sequential(nn.LayerNorm(sm_dim), nn.Linear(sm_dim, num_classes))
        self.lg_mlp_head = nn.Sequential(nn.LayerNorm(lg_dim), nn.Linear(lg_dim, num_classes))

    def forward(self, img):
        # Pass image through S-branch and L-branch image embedders
        sm_tokens = self.sm_image_embedder(img)
        lg_tokens = self.lg_image_embedder(img)

        # Pass tokens through Multi-Scale Encoder
        sm_tokens, lg_tokens = self.multi_scale_encoder(sm_tokens, lg_tokens)

        # Extract CLS tokens from S-branch and L-branch
        sm_cls, lg_cls = map(lambda t: t[:, 0], (sm_tokens, lg_tokens))

        # Pass CLS tokens through MLP heads
        sm_logits = self.sm_mlp_head(sm_cls)
        lg_logits = self.lg_mlp_head(lg_cls)

        # Combine logits from S-branch and L-branch
